...
processData<- function(inFile, p.x, multiplier, tickSize){
  #Process data using close prices as advised by Ernie
  ...
  df.temp$hHigh <- runmax(df.temp$close, p.x, align="right", endrule="keep")  #changed from high to close on Ernie's advice
  df.temp$lLow <- runmin(df.temp$close, p.x, align="right", endrule="keep")    #changed from low to close on Ernie's advice
  df.temp$hHighlLowDiff <- df.temp$hHigh - df.temp$lLow
  df.temp$relCls <- (df.temp$close - df.temp$lLow)/df.temp$hHighlLowDiff #Changed from close to pClose
  df.temp$relCls[is.na(df.temp$relCls) | is.infinite(df.temp$relCls)]<-0
  
  print("Rolling mean and SD for zScore")
  df.temp$mean<-runmean(df.temp$close, p.x, alg=c("C"),endrule=c("mean"),align = c("right"))
  
  df.temp$sd<- runsd(df.temp$close, p.x, endrule=c("sd"),align = c("right"))
  
  df.temp$zScore<-(df.temp$close-df.temp$mean)/df.temp$sd
  
  vrb<-c("datetime","open","pOpen","close","pClose","relCls","ATR", "mean", "sd", "zScore")
  return (df.temp[,vrb])
}
...
processSignals<-function(df.merged, p.x, stdev, thres, signal.type, ts, x.size, y.size, updateProgress=NULL){
  #To process signals to enter/exit trade. As advised by Ernie, I used a entry at close upon entry signal generation at close in the same bar.
  ...
  df.x$fDatetime<-c(as.POSIXlt(df.x$datetime[-1]), as.POSIXlt(df.x$datetime[1]))
  df.x$exit<-ifelse(sign(df.x$s1s2)!=c(NA,sign(df.x$s1s2)[-nrow(df.x)]),1,0)
  df.x$exit<-ifelse(day(df.x$datetime)==27 & day(df.x$fDatetime)!=27,1,df.x$exit) #Exit contract at every end of 27th day of the month
  df.x$fExit<-df.x$exit
  ...
  df.x$s1s2.signal<-ifelse(df.x$s1s2<=df.x$s1s2.bband[,"dn"], 1, ifelse(df.x$s1s2>=df.x$s1s2.bband[,"up"], 2, 0)) #Using s1s2 BBand
  ...
  vrb <- c("datetime","open.x","pOpen.x","close.x","pClose.x","open.y","pOpen.y","close.y","pClose.y" ,"s1s2","s1s2.signal","zScore","z.signal",
           "signal","fSignal","exit","fExit","ATR.ratio","s1s2.slope", "z.bband.lower","z.bband.upper","z.bband.ma")
...
backTest<- function(df.ab, ts, x.size, updateProgress=NULL){
  ...
  df.x$price.x[df.x$fEntry!=0]<-df.x$close.x[df.x$fEntry!=0]
  df.x$price.y[df.x$fEntry!=0]<-df.x$close.y[df.x$fEntry!=0]
  ...
  for (i in 2:rowCount){
    if (!inTrade) {
      ...
      if (df.x$fExit[i]==1){
        ...
        df.x$size.x[startRow:endRow]<-df.x$size.x[startRow]
        df.x$size.y[startRow:endRow]<-df.x$size.y[startRow]
  ...
  df.x$pl.x[exitTrade]<-df.x$pos.x[exitTrade]*df.x$size.x[exitTrade]*df.x$clPcl.x[exitTrade]*ts[1]
  df.x$pl.y[exitTrade]<-df.x$pos.y[exitTrade]*df.x$size.y[exitTrade]*df.x$clPcl.y[exitTrade]*ts[2]
...
processDownloadData<-function(input){
  df.eBTemp<-input
  df.eBTemp <- df.eBTx[,c("datetime","open.x","close.x","open.y","close.y", "s1s2","zScore", "z.bband.up","z.bband.dn", "pos.x", "size.x", "pos.y","size.y","price.x","price.y","pl.x","pl.y","daily.pl","total.pl","inTrade")]   
  ...
  df.eBTemp$datetime.1 <- as.POSIXct(df.eBTemp$datetime) +60
  df.eBTemp$inTrade.str<-ifelse(df.eBTemp$inTrade==2,"Enter",ifelse(df.eBTemp$inTrade==3,"Exit",ifelse(df.eBTemp$inTrade==1,"Hold","")))
  df.eBTemp<-df.eBTemp[c("datetime","datetime.1","open.x","close.x","open.y","close.y", "s1s2","zScore", "z.bband.up","z.bband.dn", "inTrade.str","pos.x.str", "size.x", "pos.y.str","size.y","price.x","price.y","pl.x","pl.y","daily.pl","total.pl")]
  colnames(df.eBTemp)<-c("Date and Time","DT1", "O1","C1", "O2","C2", "S1S2","zScore", "zScore BBand Up","zScore BBand Dn", "Signal","Pos1","Size1","Pos2","Size2","Price1","Price2","Profit1","Profit2","P/L","Total P/L")
