...
processData_ClosePricesOnly <- function(inFile, p.x, multiplier, tickSize){
...
processData_Original <- function(inFile, p.x, multiplier, tickSize){
  print("Processing Data")
  df.temp<-data.frame(readData(inFile))
...
processSignals_Ernie<-function(df.merged, p.x, stdev, thres, signal.type, ts, x.size, y.size, updateProgress=NULL){
...
processSignals_Original<-function(df.merged, p.x, stdev, thres, signal.type, ts, x.size, y.size, updateProgress=NULL){
